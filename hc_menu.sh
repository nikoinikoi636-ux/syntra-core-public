...
PS3="Choose> "
opts=("Status" "Fix (safe)" "Start Orchestrator" "Stop Orchestrator" "Tail Log" "Exit")
while true; do
  select opt in "${opts[@]}"; do
    case $REPLY in
      1) ~/WorkingProgram/HeartCore/hc_doctor_v2.sh diag; break;;
      2) ~/WorkingProgram/HeartCore/hc_doctor_v2.sh fix;  break;;
      3) nohup python3 ~/WorkingProgram/HeartCore/orchestrator.py >> ~/WorkingProgram/HeartCore/orchestrator.log 2>&1 & echo "[ok] start attempted"; break;;
